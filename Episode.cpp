...
===================
*/
bool Episode::GetNumberOfUnlockedLevels(libHolder<int> &unlocked, const char *path, int numOfLevels)
{
    unlocked.Set(1);
    ...
    if (save.IsOpened())
    {
        ...
        save.Read(&levelsSize, sizeof(libHolder<int>), 1);

        if (levelsSize.Get() == numOfLevels)
            save.Read(&unlocked, sizeof(libHolder<int>), 1);
