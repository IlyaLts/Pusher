...
bool Draw()
{
    ...
    libVec2 texCoord(libCast<float>(screenSize.x / t_background->GetWidth()),
                     libCast<float>(screenSize.y / t_background->GetHeight()));

    t_background->Draw2DQuad(libQuad(libVertex(), libVertex(libVec2(screenSize), texCoord)));

    if (menu.GetState() != Menu::IN_GAME)
        menu.Draw();
    else
        game.Draw();
...
bool Update()
{
    ...
    if (menu.GetState() != Menu::IN_GAME)
    {
        ...
        if (menu.GetState() == Menu::MAIN_MENU && engine->IsKeyPressed(LIBK_ESCAPE))
        {
            if (game.GetState() != Game::NOT_STARTED)
                menu.SetState(Menu::IN_GAME);
            else
                engine->Stop();
        }
    ...
    {
        // Opens the main menu or exits the editor play mode
        if (engine->IsKeyPressed(LIBK_ESCAPE))
        {
            if (game.GetState() == Game::EDITOR_PLAYTEST)
            {
