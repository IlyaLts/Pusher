...
void Game::Draw()
{
    ...
    for (int i = 0; i < LEVEL_WIDTH; i++)
    {
        for (int j = 0; j < LEVEL_HEIGHT; j++)
        {
            ...
            switch (GetLevel().GetBlock(i, j).type)
            {
                ...
                {
                    t_endpoint->Draw2DQuad(block, x, y);
                    block.Set(darkenedColor);
                    t_box->Draw2DQuad(block, x, y);
                    block.Set(defaultColor);
                    break;
                }
    ...
    if (IsCursorInField())
    {
        ...
        {
            block.Set(transparentColor);

            switch (currentBlock)
            {
            case Block::ROBOT:
            {               
                t_robot->Draw2DQuad(block, hoverOver.x, hoverOver.y);
                break;
            }
            case Block::BOX:
            {
                t_box->Draw2DQuad(block, hoverOver.x, hoverOver.y);
                break;
            }
            case Block::WALL:
            {
                t_wall->Draw2DQuad(block, hoverOver.x, hoverOver.y);
                break;
            }
            case Block::ENDPOINT:
            {
                t_endpoint->Draw2DQuad(block, hoverOver.x, hoverOver.y);
                break;
            }
            case Block::BOX_ON_ENDPOINT:
            {
