...
bool Game::Init()
{
    ...
    t_robot->SetDuration(ROBOT_ANIMATION_DURATION);
    t_robot->Play();
...
void Game::Draw()
{
    if (state == EDITOR_NEW_NAME)
    {
        ...
        if (episodeAlreadyExists)
        {
            ...
            float y = screenSize.y - font->GetLineSpacing();
    ...
    libVec2 hoverOver(gameScreenPos.x + mouse.x * tileSize.x, gameScreenPos.y + mouse.y * tileSize.y);
...
#else
    if (engine->IsKey(LIBK_CTRL) && (GetState() == EDITOR || GetState() == EDITOR_PLAYTEST))
#endif
    {
        if (engine->IsKeyPressed(LIBK_G))
        {
            SetState((GetState() == IN_GAME || state == EDITOR_PLAYTEST) ? EDITOR : EDITOR_PLAYTEST);
        }
    }
...
void Game::GameUpdate()
{
    ...
    auto &current = level.GetBlock(currentCoord.x, currentCoord.y).type;

    engine->SetState(LIB_WINDOW_TITLE, libFormat("Pusher - %s - Level: %d", episode.name.Get(), currentLevel));

#ifdef LIB_DEBUG
    if (level.IsCompleted() || (engine->IsKey(LIBK_CTRL) && engine->IsKeyPressed(LIBK_ENTER)))
#else
    ...
    // Moves the robot up
    if (robot.y > 0 && ((path[robot.x][robot.y - 1] > path[robot.x][robot.y] && canMoveByMouse) || moveKeyDir == UP))
    {
        moveTimer.Restart();
        ...
        if (target.CanMoveBoxes())
        {
            ...
            if (current == Block::ROBOT)
                level.SetBlockType(currentCoord.x, currentCoord.y, Block::EMPTY);

            if (current == Block::ROBOT_ON_ENDPOINT)
                level.SetBlockType(currentCoord.x, currentCoord.y, Block::ENDPOINT);
    ...
    else if (robot.y < LEVEL_HEIGHT - 1 && ((path[robot.x][robot.y + 1] > path[robot.x][robot.y] && canMoveByMouse) || moveKeyDir == DOWN))
    {
        moveTimer.Restart();
    ...
    else if (robot.x > 0 && ((path[robot.x - 1][robot.y] > path[robot.x][robot.y] && canMoveByMouse) || moveKeyDir == LEFT))
    {
        moveTimer.Restart();
    ...
    else if (robot.x < LEVEL_WIDTH - 1 && ((path[robot.x + 1][robot.y] > path[robot.x][robot.y] && canMoveByMouse) || moveKeyDir == RIGHT))
    {
        moveTimer.Restart();
...
{
    moveDirection_t moveKeyDir = NEUTRAL;
    static moveDirection_t movePressedKeyDir = NEUTRAL;
    static libVec2i mouseTarget(-1, -1);

    canMoveByMouse = (moveTimer.GetMilliseconds() >= moveMouseDelay);
